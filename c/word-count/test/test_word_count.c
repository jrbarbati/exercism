                           ...
                           int actual_word_count)
{
    ...
    {
        int expected_count = expected_solution[index].count;
        int actual_count = actual_solution[index].count;
        char *expected_string = expected_solution[index].text;
        char *actual_string = actual_solution[index].text;

        printf("EXPECTED -- %s : %d\n", expected_string, expected_count);
        printf("ACTUAL   -- %s : %d\n", actual_string, actual_count);

        TEST_ASSERT_EQUAL(expected_count,
                          actual_count);
        TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_string,
                                      actual_string, STRING_SIZE);
    }
}
